...
def main() -> None:
    ...
    logistic_model = train_logistic_regression(x_train, y_train, settings)
    print(json.dumps(evaluate_model(logistic_model, x_test, y_test), indent=2))
    print()
    print("Random Forest metrics:")
    random_forest_model = train_random_forest(x_train, y_train, settings)
    print(json.dumps(evaluate_model(random_forest_model, x_test, y_test), indent=2))
